...
# تحميل الصفقات السابقة من قاعدة البيانات عند الإقلاع
...
<html lang="ar" dir="rtl">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>التداول اليدوي والتحليلات</title>
<style>
...
.btn{padding:8px 12px;border:none;border-radius:6px;font-weight:bold;cursor:pointer;font-size:12px}
input,select{background:#090d16;border:1px solid var(--border);color:#fff;padding:8px;border-radius:6px;font-size:12px;width:100%}
.pct-btn{background:#151e30;border:1px solid var(--border);color:var(--sub);padding:4px 8px;border-radius:6px;cursor:pointer;font-size:11px}
.pct-btn:hover{background:#3b82f6;color:#fff}
</style>
</head>
<body>
  <div class="card" style="display:flex;justify-content:space-between;align-items:center">
    <div>
      <strong>📊 مركز التحليلات والتداول اليدوي المباشر</strong>
      <div style="font-size:11px;color:var(--sub)">USDT المتاح: <strong id="m-usdt" style="color:#10b981">0.00 $</strong></div>
    </div>
    <a href="/" class="btn" style="background:#334155;color:#fff;text-decoration:none">🔙 الرئيسية</a>
  </div>

  <details open class="card">
    <summary style="font-weight:bold;cursor:pointer;color:#38bdf8;padding:4px">⚡ لوحة التداول الفوري المباشر (Spot Manual Terminal) ▾</summary>
    <div style="padding-top:10px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
        <div>
          <label style="font-size:11px;color:var(--sub)">رمز العملة:</label>
          <input type="text" id="t-sym" placeholder="مثال: SOLUSDT" value="SOLUSDT">
        </div>
        <div>
          <label style="font-size:11px;color:var(--sub)">نوع الأمر:</label>
          <select id="t-type" onchange="toggleLimitPrice()">
      ...
      <div id="limit-price-box" style="display:none;margin-bottom:8px">
        <label style="font-size:11px;color:var(--sub)">سعر التنفيذ المطلوب ($):</label>
        <input type="number" id="t-price" placeholder="أدخل السعر">
      </div>

      <div style="margin-bottom:8px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <label style="font-size:11px;color:var(--sub)">الكمية أو القيمة بالدولار ($):</label>
        ...
        <input type="number" id="t-qty" placeholder="القيمة بـ USDT للشراء أو كمية العملة للبيع">
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <button class="btn" style="background:#10b981;color:#fff" onclick="execTrade('BUY')">🟢 تنفيذ شراء</button>
        <button class="btn" style="background:#ef4444;color:#fff" onclick="execTrade('SELL')">🔴 تنفيذ بيع</button>
      </div>
    </div>
  </details>
...
function setPct(p){
  document.getElementById('t-qty').value = (freeUsdt * p).toFixed(2);
}
async function execTrade(side){
  ...
  if(!sym || !val){ alert('يرجى ملء رمز العملة والكمية'); return; }
  if(type === 'LIMIT' && (!price || price <= 0)){ alert('يرجى تحديد سعر الأمر'); return; }
  ...
  if(confirm(`تأكيد إرسال أمر ${side} لـ ${payload.symbol} بقيمة/كمية ${val}؟`)){
...
<body>
  <div class="header-box">
    <div>
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <a href="/analytics" style="color:#60a5fa;font-size:12px;text-decoration:none;background:#1e293b;padding:2px 8px;border-radius:6px">📊 التحليلات واليدوي ↗</a>
      </div>
  ...
  <div id="t1" class="tab-pane active">
    ...
    <div class="card" id="b1-coins-card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        <div style="display:flex;gap:4px">
          ...
          <button class="icon-btn" style="background:#334155;color:#fff;font-size:12px" title="تبديل وضع الإدارة" onclick="toggleManage('b1-coins-card')">⚙️</button>
        </div>
      </div>
      <div style="overflow-x:auto"><table id="b1-coins-table"><thead><tr><th>العملة</th><th>السعر</th><th>ربح اليوم</th><th>الصفقات</th><th>دخول</th><th class="manage-ctrl">إجراء</th></tr></thead><tbody></tbody></table></div>
    </div>

    <div class="card">
      <strong style="font-size:12px;display:block;margin-bottom:4px">📂 الصفقات المفتوحة (Live PnL & Persistence):</strong>
      <div style="overflow-x:auto"><table id="b1-orders"><thead><tr><th>العملة</th><th>الدخول</th><th>الكمية</th><th>Live PnL</th><th>الوقت</th><th>إجراءات</th></tr></thead><tbody></tbody></table></div>
    </div>
  </div>
  ...
  <div id="t2" class="tab-pane">
    ...
    <div class="card" id="b2-coins-card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <strong style="font-size:12px">📊 جدول العملات وأرباح اليوم:</strong>
        <div style="display:flex;gap:4px">
          ...
          <button class="icon-btn" style="background:#334155;color:#fff;font-size:12px" title="تبديل وضع الإدارة" onclick="toggleManage('b2-coins-card')">⚙️</button>
        </div>
      </div>
      <div style="overflow-x:auto"><table id="b2-coins-table"><thead><tr><th>العملة</th><th>السعر</th><th>ربح اليوم</th><th>الصفقات</th><th>دخول</th><th class="manage-ctrl">إجراء</th></tr></thead><tbody></tbody></table></div>
    </div>

    <div class="card">
      <strong style="font-size:12px;display:block;margin-bottom:4px">📂 الصفقات المفتوحة (Live PnL & Persistence):</strong>
      <div style="overflow-x:auto"><table id="b2-orders"><thead><tr><th>العملة</th><th>الدخول</th><th>الكمية</th><th>Live PnL</th><th>الوقت</th><th>إجراءات</th></tr></thead><tbody></tbody></table></div>
    </div>
  </div>
  ...
  <div id="t3" class="tab-pane">
    ...
    <div class="card" id="b3-coins-card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        <div style="display:flex;gap:4px">
          ...
          <button class="icon-btn" style="background:#334155;color:#fff;font-size:12px" title="تبديل وضع الإدارة" onclick="toggleManage('b3-coins-card')">⚙️</button>
        </div>
      </div>
      <div style="overflow-x:auto"><table id="b3-market-table"><thead><tr><th>العملة</th><th>السعر</th><th>إطلاق بنقرة</th><th class="manage-ctrl">إجراء</th></tr></thead><tbody></tbody></table></div>
    </div>

    <div class="card">
      <strong style="font-size:12px;display:block;margin-bottom:4px">📂 صفقات Bot 3 المفتوحة (Live PnL & Persistence):</strong>
      <div style="overflow-x:auto"><table id="b3-orders"><thead><tr><th>العملة</th><th>الدخول</th><th>أعلى سعر</th><th>الكمية</th><th>Live PnL</th><th>الوقت</th><th>إجراءات</th></tr></thead><tbody></tbody></table></div>
    </div>
  </div>
  ...
  <div id="t4" class="tab-pane">
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px">
        ...
        <div style="display:flex;gap:4px">
          <button class="btn" style="background:#8b5cf6;color:#fff;font-size:11px" onclick="convertDustDirect()">🔄 تحويل الأرصدة الصغيرة لـ MX</button>
    ...
    <!-- الأوامر المعلقة في المنصة -->
    <div class="card">
      <strong style="font-size:12px;display:block;margin-bottom:6px">⏳ الأوامر المعلقة في المنصة (Limit Orders):</strong>
      <div style="overflow-x:auto">
        <table id="limit-orders-table">
          <thead><tr><th>العملة</th><th>النوع</th><th>السعر المطلوب</th><th>الكمية</th><th>الوقت</th><th>إلغاء الأمر</th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
    </div>
...
function toggleManage(cardId){
  document.getElementById(cardId).classList.toggle('manage-mode');
}
  ...
  try{
    ...
    ['BOT_1', 'BOT_2', 'BOT_3'].forEach(bKey => {
      ...
      (bObj.symbols || []).forEach(sym => {
        ...
        coinsTableHtml += `<tr>
          ...
          <td class="manage-ctrl"><button class="icon-btn" style="background:#334155;color:#f87171" title="حذف من الجدول" onclick="removeCoinFromBot('${bKey}','${sym}')">🗑️</button></td>
        </tr>`;
      });
      ...
      for(let s in bObj.active_positions){
        const curBid = d.market_prices[s] ? d.market_prices[s].bid : 0.0;
        (bObj.active_positions[s] || []).forEach(p=>{
          const livePnlVal = curBid ? (curBid - p.entry_price) * p.qty : 0.0;
          const livePnlPct = curBid ? ((curBid - p.entry_price) / p.entry_price) * 100.0 : 0.0;
          ...
          posHtml += `<tr>
            ...
            <td>
              <div style="display:flex;gap:3px">
                <button class="icon-btn" style="background:var(--danger);color:#fff" title="تسييل بيع فوري" onclick="closeSinglePos('${bKey}','${s}', '${p.id}')">🔥</button>
                <button class="icon-btn" style="background:#3b82f6;color:#fff" title="تعديل الصفقة" onclick="openEditModal('${p.id}', ${p.entry_price}, ${p.qty}, ${p.tp_pct||0.015}, ${p.sl_pct||0.005})">✏️</button>
                <button class="icon-btn" style="background:#475569;color:#fca5a5" title="إلغاء من البوت دون بيع" onclick="unlinkPos('${bKey}','${s}', '${p.id}')">🚫</button>
              </div>
            </td>
          </tr>`;
        });
    ...
    (d.bots["BOT_3"].symbols || []).forEach(sym => {
      ...
      b3MHtml += `<tr>
        ...
        <td class="manage-ctrl"><button class="icon-btn" style="background:#334155;color:#f87171" title="حذف من القائمة" onclick="removeCoinFromBot('BOT_3','${sym}')">🗑️</button></td>
      </tr>`;
    });
    ...
    (d.open_limit_orders || []).forEach(o => {
      ordHtml += `<tr>
        ...
        <td><button class="icon-btn" style="background:#dc2626;color:#fff" title="إلغاء الأمر" onclick="cancelLimitOrder('${o.symbol}', '${o.orderId}')">❌</button></td>
      </tr>`;
    });
    document.getElementById('limit-orders-table').querySelector('tbody').innerHTML = ordHtml || '<tr><td colspan="6" style="text-align:center;color:var(--sub)">لا توجد أوامر معلقة في المنصة</td></tr>';
